...
option = gets.chomp.to_i
  ...
  if option == 1
  #1 print out list of animals in the shelter
  # shelter.each{|k,v| p v[:animals]}
  # shelter[:animal].each{|k,v|}
  # @name.each do|animal|
  #   puts animal.get_animal_name
  # end
  # shelter[:animals].each do

  for i in 0..shelter[:animals].length-1
    if shelter[:animals][i].get_animal_gender == 'f'
      puts "#{shelter[:animals][i].get_animal_name} is a female #{shelter[:animals][i].get_animal_species} aged #{shelter[:animals][i].get_animal_age}, she comes with #{shelter[:animals][i].get_animal_toys}"
    else
      puts "#{shelter[:animals][i].get_animal_name} is a male #{shelter[:animals][i].get_animal_species} aged #{shelter[:animals][i].get_animal_age}, he comes with #{shelter[:animals][i].get_animal_toys}"
    end
  end
  ...
  for i in 0..shelter[:clients].length-1
    puts "#{shelter[:clients][i].get_client_name}, #{shelter[:clients][i].get_client_age}, has #{shelter[:clients][i].get_client_kids} kids and #{shelter[:clients][i].get_client_pets} pets"
  end
...
# not working yet
  ...
  #5 for adoption, remove animal from shelter and add it to client
    ...
    lucky = gets.chomp
    aid = shelter[:animals].index(lucky)
    shelter[:animals][aid].delete
