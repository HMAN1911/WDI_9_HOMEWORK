...
function withdraw(account,amount) {
  ...
  if (userAccounts[current][account]>=amount) {
    ...
    var otherAccount = (account+1)%2;
    if (userAccounts[current][account]+userAccounts[current][otherAccount]>=amount) {
      userAccounts[current][otherAccount]-=(amount-userAccounts[current][account]);
      userAccounts[current][account]=0;
      red(account);
      if (userAccounts[current][otherAccount]===0) red(account);
