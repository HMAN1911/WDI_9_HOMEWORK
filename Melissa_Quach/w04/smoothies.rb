...
class Blender
  # Blender should be off by default
  def initialize
    @on = false
  end

  # Assuming that ingredients is an array of ingredient names
  def blend(ingredients)
    # Prevent blender from blending if off
    if !@on
      return "Blender is currently off"
    else
      return ingredients
        .join('')
        .split('')
