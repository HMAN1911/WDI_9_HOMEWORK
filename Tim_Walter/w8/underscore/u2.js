...
var x = _.sortBy(people, function(person){ return person.uid; });
...
var x = _.groupBy(words, function(letter){
  return letter.charAt(0).toLowerCase();
});

_.each(x, function(elem){
  console.log(elem);
});

// Check to see if all the words have more than 3 characters
x = _.filter(words, function(letter) {
...
var sortByUids =_.sortBy(people, function(person){ return person.uid})
var sortByUids = sortByUids.reverse();
console.log(sortByUids[0].uid);

// Return an object that says how many even numbers and how many odd numbers there are in numbers


var x = _.partition(numbers, function(number){
  if (number%2 == 0) {
    return true
  };
})
console.log("evens " + x[0]);
console.log("odds " + x[1]);
